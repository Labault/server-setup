...
write_server_state() {
  ...
  tmp="$(mktemp "$STATE_DIR/state.yaml.XXXXXX")" || die "cannot write state in ${STATE_DIR}"
  {
    ...
    printf 'paranoid: %s\n' "${PARANOID:-0}"
    # The non-root sudoer's name (--deploy-user). Every deploy-user predicate
    # keys off it, so doctor MUST read back the name setup converged — otherwise
    # it would look for `deploy` on a box that has none and report phantom drift.
